...
def _mk_butter_filter(freq, type, order):
    ...
    Args:
        ...
        order: nth order butterworth filter (default: 5th order). The
            attenuation is -6dB/octave beyond the cutoff frequency (for
            1st order). A Higher order filter will have more attenuation,
            each level adding an additional -6dB (so a 3rd order
            butterworth filter would be -18dB/octave).
...
def _eq(seg, focus_freq, bandwidth=100, mode="peak", gain_dB=0, order=2):
    ...
    Args:
        ...
        mode - Mode of Equalization(Peak/Notch(Bell Curve), High Shelf,
            Low Shelf)
...
def eq(
    ...
    Args:
        ...
        filter_mode - Mode of Equalization(Peak/Notch(Bell Curve),
